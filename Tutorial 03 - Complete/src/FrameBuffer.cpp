...
FrameBuffer::FrameBuffer(uint32_t width, uint32_t height, uint8_t numSamples) {
	...
	if (myNumSamples > 1) { myUnsampledFrameBuffer = std::make_shared<FrameBuffer>(width, height, 1); }
}
...
void FrameBuffer::Resize(uint32_t newWidth, uint32_t newHeight) {
	...
	if (myNumSamples > 1) { myUnsampledFrameBuffer->Resize(newWidth, newHeight); }
}
...
void FrameBuffer::AddAttachment(const RenderBufferDesc& desc)
{
	...
	if (buffer.IsRenderBuffer) {
		...
		//glEnable(GL_FRAMEBUFFER_SRGB); //this does gama correction when enabled
	}
	...
	isValid = false;
	if (myNumSamples > 1) { myUnsampledFrameBuffer->AddAttachment(desc); }
}
